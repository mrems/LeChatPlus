...
export async function renderFolders(): Promise<void> {
  ...
  // Créer et ajouter chaque dossier à la liste
  for (const folder of folders) {
    ...
    if (conversations.length > 0) {
      for (const conv of conversations) {
        ...
        safeSetStyle(convItem, 'marginBottom', '1px');
        ...
        safeSetStyle(actionButtons, 'height', '18px'); // Réduire la hauteur des boutons d'action
...
export async function injectFoldersUI(): Promise<void> {
  ...
  if (!sidebarElement) {
    ...
    // Dernier recours: injecter au début du body avec un délai
    setTimeout(() => {
      if (!document.getElementById('le-chat-plus-folders')) {
        console.log("Tentative d'injection directe dans le body");
        const bodyFirstChild = document.body.firstChild;
        if (bodyFirstChild) {
          // Créer un conteneur pour l'extension
          const extensionContainer = document.createElement('div');
          extensionContainer.id = 'le-chat-plus-container';
          safeSetStyle(extensionContainer, 'position', 'fixed');
          safeSetStyle(extensionContainer, 'top', '60px'); // En dessous de la barre de navigation
          safeSetStyle(extensionContainer, 'left', '10px');
          safeSetStyle(extensionContainer, 'width', '280px');
          safeSetStyle(extensionContainer, 'zIndex', '1000');
          safeSetStyle(extensionContainer, 'background', 'var(--background-color, white)');
          safeSetStyle(extensionContainer, 'borderRadius', '8px');
          safeSetStyle(extensionContainer, 'boxShadow', '0 2px 8px rgba(0, 0, 0, 0.1)');
          safeSetStyle(extensionContainer, 'padding', '10px');
          
          document.body.insertBefore(extensionContainer, bodyFirstChild);
          injectFoldersUI(); // Rappeler la fonction, qui trouvera maintenant notre conteneur
        }
      }
    }, 2000);
